...
Td = xl_file[72:73].values.tolist()[0][1]
Td = int(Td[Td.find('=')+1:Td.find('minutes')])
m.addConstr( (gb.quicksum(gb.quicksum(mij[i][j] * d[i][j] for i in range(5)) for j in range(5)))/5 + (gb.quicksum(gb.quicksum(mij[i][j] * times[j - 1] for i in range(5)) for j in range(1,5))) <= Td)
